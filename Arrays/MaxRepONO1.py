...
k <= n. Find the maximum repeating number in this array. For example, let k be 10 the given array be
 ...
 Expected time complexity is O(n) and extra space allowed is O(1). Modifications to array are
 allowed.

 A better approach is to create a count array of size k and initialize all elements of count[]
 as 0. Iterate through all elements of input array, and for every element arr[i], increment
 count[arr[i]]. Finally, iterate through count[] and return the index with maximum value.
...
2) Find the maximum value in the modified array (maximum value is 29). Index of the maximum value
is the maximum repeating element (index of 29 is 3).

3) If we want to get the original array back, we can iterate through the array one more time
 and do arr[i] = arr[i] % k where i varies from 0 to n-1.
